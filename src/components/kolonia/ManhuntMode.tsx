  ...
  return (
    <span
      ...
      className={`inline-block size-3 rounded-full border ${
        filled
          ? "border-sky-400/80 bg-sky-500 shadow-[0_0_6px_rgba(56,189,248,0.65)]"
          : "border-[var(--panel-ink)]/25 bg-[var(--panel-ink)]/10"
      }`}
    />
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
        <article className="relative border-2 border-[var(--panel-ink)]/35 bg-[var(--panel)]/55 p-4 shadow-inner sm:p-5">
          <div className="mb-3 flex items-start justify-between gap-3 border-b border-[var(--panel-ink)]/20 pb-3">
            <div>
              <div className="font-mono text-[10pt] uppercase tracking-[0.14em] text-[var(--rust)]">
                {mh.wanted} №{puzzle}
              </div>
              <p className="mt-2 font-mono text-[10pt] uppercase tracking-[0.12em] text-[var(--panel-ink)]">
                {mh.escapedFrom}:{" "}
                <span className="text-[var(--rust)]">
                  {manhuntFieldValue("camp", targetNpc, targetQuote, lang)}
                </span>
              </p>
            </div>
            <span aria-hidden="true" className="text-2xl text-[var(--rust)]">
              ☧
            </span>
          </div>

          <div className="mx-auto max-w-[220px]">
            <div className="relative aspect-[3/4] overflow-hidden border border-[var(--panel-ink)]/30 bg-[var(--panel-ink)]/10">
              {portraitUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img alt="" className="size-full object-cover object-top opacity-90" src={portraitUrl} />
              ) : (
              ...
              {state.status === "won" ? (
                <div
                  className="pointer-events-none absolute inset-0 flex items-center justify-center bg-[var(--rust)]/10"
                  aria-hidden="true"
                >
                  <span className="rotate-[-12deg] border-4 border-[var(--rust)] px-4 py-2 font-mono text-2xl uppercase tracking-[0.2em] text-[var(--rust)]">
                    {mh.captured}
                  </span>
                </div>
              ) : null}
        ...
        <div className="space-y-4">
          <div className="border border-[var(--panel-ink)]/20 bg-[var(--panel)]/35 p-3">
            <div className="mb-2 font-mono text-[10pt] uppercase tracking-[0.14em] text-[var(--panel-ink)]/70">
              {mh.purse}
            </div>
          ...
          <div className="space-y-2">
            {MANHUNT_CONFIG.fields.map((field) => {
              ...
              return (
                <div
                  className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-[var(--panel-ink)]/15 py-2 last:border-b-0"
                  key={field.id}
                >
                  <div className="min-w-0">
                    <div className="font-mono text-[9pt] uppercase tracking-[0.12em] text-[var(--panel-ink)]/55">
                      {label}
                    </div>
                    {revealed ? (
                      <div className="mt-1 text-sm italic leading-snug text-[var(--panel-ink)]">{value}</div>
                    ) : null}
                  </div>
                  {!revealed ? (
                    <button
                      className="shrink-0 border border-[var(--panel-ink)]/40 px-3 py-2 font-mono text-[9pt] uppercase tracking-[0.1em] text-[var(--panel-ink)] transition-colors hover:border-[var(--rust)] hover:text-[var(--rust)] disabled:cursor-not-allowed disabled:opacity-40"
      ...
      {missNames.length > 0 ? (
        <div className="border border-dashed border-[var(--panel-ink)]/25 bg-[var(--panel)]/25 p-3">
          ...
          <p className="font-serif text-base italic text-[var(--panel-ink)]/75">
            {missNames.map((name) => (
              <span className="mr-3 line-through" key={name}>
                {name}
              </span>
            ))}
          </p>
