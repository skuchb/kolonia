...
export const MANHUNT_CONFIG = {
  ...
  fields: [
    ...
    { id: "location" as const, cost: 2 },
    { id: "letter" as const, cost: 2 },
    { id: "teacher" as const, cost: 1 },
    { id: "trade" as const, cost: 1 },
    { id: "quote" as const, cost: 3 },
  ],
} as const;

const paidSum = MANHUNT_CONFIG.fields.reduce((sum, field) => sum + field.cost, 0);
if (paidSum <= MANHUNT_CONFIG.budget) {
  throw new Error(`manhunt: paid field costs (${paidSum}) must exceed budget (${MANHUNT_CONFIG.budget})`);
}
